...
### Tables (critical — legibility trap)

**The real problem — zebra striping on dark slides.** Marp's built-in
`theme: default` inherits GitHub-Flavored Markdown CSS, which includes:

```css
tr                   { background-color: var(--bgColor-default); border-top: 1px solid var(--borderColor-muted); }
tr:nth-child(2n)     { background-color: var(--bgColor-muted); }
```

In GitHub light mode these are off-white and light-gray — the familiar
zebra-striped README tables. When you paint a slide black, the row
backgrounds **do not change** — they still render near-white. So you
get alternating white bands with your white text on top. Every other
row becomes unreadable. This is the bug users report as "can't see
the table on slide X."

There is also a secondary issue where `color: light-dark(#246,...)` can
leak in on `h1` and links, but the **dominant** failure mode is zebra
striping on black slides.

**The fix — zero out row backgrounds AND borders with `!important`.**

```css
/* Base table (light slides) */
table {
  width: 100%;
...
tbody, thead, tfoot { background: transparent !important; }
...
td em, th em         { color: inherit !important; opacity: 0.7; }
td strong, th strong { color: inherit !important; }
tr:last-child td     { border-bottom: none !important; }

/* Dark slides — ALL overrides declared AFTER the base */
section.dark table { color: #ffffff !important; background: transparent !important; }
section.dark tbody, section.dark thead, section.dark tfoot {
  background: transparent !important;
}
section.dark tr, section.dark tr:nth-child(2n),
section.dark tr:nth-child(odd), section.dark tr:nth-child(even) {
  background: transparent !important;
  background-color: transparent !important;
}
section.dark th {
...
**Why you cannot rely on inline `<tr style>` or `<td style>` attributes.** Marp's markdown-it table parser **strips HTML attributes from `<tr>`, `<td>`, `<th>`, and inline elements inside them** (`<strong>`, `<em>`). It re-emits clean markdown-table HTML with no attributes. Any styling applied via inline `style=` gets thrown away. Always use CSS rules in the frontmatter `style:` block, never inline styles on table cells.

**De-emphasizing rows.** Since you can't use inline styles on rows, define utility classes in your frontmatter and apply them via `<tr class="muted-row">` — Marp's parser DOES preserve `class` attributes. Example:

```css
section.dark tr.muted-row td { color: rgba(255,255,255,0.55) !important; }
section.dark tr.muted-row td strong { color: #ffffff !important; }
```

Never use `opacity: 0.5–0.6` — it double-discounts text already muted by its semantic role and often crosses the legibility threshold.

**Verification step — always sanity-check.** After any CSS change affecting tables, render the deck to PNG and eyeball each dark slide:
```bash
npx --yes @marp-team/marp-cli deck.md --images png --image-scale 1
```
Inspect `deck.008.png`, `deck.011.png`, etc. Do not trust the HTML output alone — browsers cache aggressively and you may be looking at a stale render.

**Minimum sizes.** Apple body tables live at 14–16px. Go smaller than 14px only for footnote-like reference tables — below that, legibility cliff.
...
- **Always set explicit text colors on `td` and `th`** — never inherit. In Marp (and most slide renderers) the default theme leaks a lighter color into table cells, producing light-on-light illegibility on `#f5f5f7` slides. See "Tables" below.
...
- **Don't color `em` / italics blue globally** — it looks like a link, and inside tables it makes italicized cells look washed out on light backgrounds. Italicize by weight / font-style only; restrict blue to explicit `<a>` and CTAs.
- **Don't rely on `opacity:` to de-emphasize rows** on light backgrounds — 0.5–0.6 opacity on `#1d1d1f` text gives the same light-gray appearance that triggers the "I can't read this" problem. Use `color: rgba(0,0,0,0.55)` explicitly instead, which at least anchors the intended tone.
